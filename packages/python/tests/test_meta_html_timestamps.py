...
@pytest.fixture(
    params=[eh.meta_html_timestamp_to_datetime_string, instagram._html_timestamp],
    ids=["facebook", "instagram"],
)
def platform(request):
    """One reader serves both: Facebook calls the shared parser directly (its clock is
    left as is), Instagram wraps it to apply its fixed offset. Both are checked."""
...
class TestFacebookIsNotConverted:
    ...
    def test_only_the_shape_changes(self, shown, expected):
        assert eh.meta_html_timestamp_to_datetime_string(shown) == expected
    ...
    def test_it_does_not_land_where_the_json_does(self):
        ...
        from_html = eh.meta_html_timestamp_to_datetime_string(FACEBOOK_SHOWS)
    ...
    def test_every_html_extractor_with_a_date_column_reads_it(self):
        ...
        for module in (facebook, instagram):
            ...
            for part in re.split(r"(?=\ndef )", source):
                ...
                converts = re.search(r"_html_timestamp\(|_section_timestamp\(", live)
