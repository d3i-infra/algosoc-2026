...
- One cross-cutting content canary per platform is also allowed when it checks a shared helper's contract rather than a table's schema (Facebook: every `Timestamp`/`Date`/`Created` value is ISO 8601, newest first). It skips without fixtures and names the failing table only as a pointer to inspection.
